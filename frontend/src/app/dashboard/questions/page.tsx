...
export default function UserQuestionsPage() {
  ...
  useEffect(() => {
    if (!isLoading) {
      if (!user) {
        router.push("/login?redirect=/dashboard/questions");
      } else {
        loadQuestions();
      }
    }
  ...
  const loadQuestions = async () => {
    try {
      setLoading(true);
      setErrorMsg(null);
      const data = await getQuestions(search, "personal");
      setQuestions(data);
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : "Failed to load personal questions.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmSingleDelete = async () => {
    ...
    try {
      ...
      setQuestions(questions.filter((q) => q.id !== singleDeleteTarget.id));
      setSelectedIds(selectedIds.filter((id) => id !== singleDeleteTarget.id));
      setSingleDeleteTarget(null);
      setSuccessMsg("Question deleted from your Question Bank.");
  ...
  const handleConfirmBulkDelete = async () => {
    ...
    try {
      ...
      setSuccessMsg(res.message || `${count} questions deleted.`);
      setTimeout(() => setSuccessMsg(null), 3000);
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : "Failed to delete selected questions");
    } finally {
      setIsProcessingDelete(false);
    }
  };
  ...
  const handleConfirmDeleteAll = async () => {
    try {
      ...
      setSuccessMsg("All your personal questions have been deleted.");
      setTimeout(() => setSuccessMsg(null), 3000);
    } catch (err: unknown) {
  ...
  const handleStartExamWithSelected = async () => {
    if (selectedIds.length === 0) {
      alert("Please select at least 1 question to start a practice test.");
      return;
    }
    try {
      const exam = await createExam({
        title: `Practice Test (${selectedIds.length} Questions)`,
      ...
      alert(err instanceof Error ? err.message : "Failed to start practice test");
    }
  };
  ...
  const handleCreateQuestionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewQAnswers(["A", "B"]);
      setNewQExplanation("");
      setSuccessMsg("Question created successfully in your personal bank.");
      setTimeout(() => setSuccessMsg(null), 3000);
      loadQuestions();
    } catch (err: unknown) {
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Manage your personal repository of questions extracted from study documents or added manually.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          {selectedIds.length > 0 && (
            ...
            >
              ...
              Practice Test ({selectedIds.length})
            </button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-500 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="text-sm">Loading your personal questions...</p>
        </div>
      ) : questions.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-1 text-xs text-slate-500 max-w-sm mx-auto">
            Upload your first study document to extract questions or add practice questions manually.
          </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900 space-y-4">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
              Delete Question #{singleDeleteTarget.id}?
            </h3>
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Are you sure you want to remove this question from your personal Question Bank?
            </p>
      ...
      {isBulkDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900 space-y-4">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Are you sure you want to remove the {selectedIds.length} selected questions from your
      ...
      {isDeleteAllModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900 space-y-4 border-2 border-rose-500">
            <h3 className="text-base font-bold text-rose-600">
              Delete All Personal Questions?
            </h3>
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Are you sure you want to delete all {questions.length} questions in your personal
              Question Bank? This action cannot be undone.
            </p>
            <div className="flex justify-end gap-3 pt-3 border-t dark:border-slate-800">
              <button
                type="button"
              ...
              <div>
                ...
                <div className="space-y-2">
                  {editingQuestion.options.map((opt, oIdx) => {
                    const letter = String.fromCharCode(65 + oIdx);
                    const isCorrect = editingQuestion.correct_answers.includes(letter);
                    return (
                      <div key={oIdx} className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            const answersSet = new Set(editingQuestion.correct_answers);
            ...
            <form onSubmit={handleCreateQuestionSubmit} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  required
                  value={newQText}
                  placeholder="e.g. Which of the following are cybersecurity frameworks?"
